...
public class OverpassClient {
    ...
    private CompletableFuture<List<OsmFeature>> tryEndpoint(String query, String bboxKey, int endpointIndex) {
        ...
        // Raised alongside the query's own [timeout:180] budget above -- a 2km+ diameter bounding
        // box in a dense city can return a lot more data than the 300m-radius case this was
        // originally tuned for, so the HTTP client needs headroom past the server-side budget
        // rather than cutting the connection before Overpass itself would have given up.
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create((String)endpoint)).header("Content-Type", "application/x-www-form-urlencoded").header("User-Agent", "NexusTerra/0.1.7 (Minecraft terrain generator)").timeout(Duration.ofSeconds((long)195L)).POST(HttpRequest.BodyPublishers.ofString((String)("data=" + URLEncoder.encode((String)query, (Charset)StandardCharsets.UTF_8)))).build();
    ...
    private Classification classify(JsonObject tags) {
        ...
        if (tags.has("man_made")) {
            String manMadeValue = this.safeString(tags, "man_made", "");
            // Routed into the BUILDING category with a distinctive subtype rather than a new
            // Category enum value -- the building loop already does everything a real category
            // split would need (footprint rasterising, radius clipping, per-building hashing),
            // it just special-cases these two subtypes before falling into ordinary wall-building.
            if ("canopy".equals(manMadeValue)) {
                return new Classification(OsmFeature.Category.BUILDING, "canopy");
            }
            if ("storage_tank".equals(manMadeValue)) {
                return new Classification(OsmFeature.Category.BUILDING, "storage_tank");
            }
        }
        if (tags.has("highway")) {
            String highwayValue = this.safeString(tags, "highway", "road");
            // A pedestrian/footway/living_street way tagged area=yes is a paved plaza or square,
            // not a linear road -- this is the standard real-world OSM tagging for places like
            // Trafalgar Square. Route it into the LANDUSE pipeline (polygon fill, plaza surface,
            // fountain-eligible) instead of rasterizing it as a thin road strip.
            boolean isArea = "yes".equals(this.safeString(tags, "area", "no"));
            if (isArea && ("pedestrian".equals(highwayValue) || "footway".equals(highwayValue) || "living_street".equals(highwayValue))) {
                return new Classification(OsmFeature.Category.LANDUSE, "pedestrian");
            }
            return new Classification(OsmFeature.Category.ROAD, highwayValue);
        }
        if (tags.has("railway")) {
