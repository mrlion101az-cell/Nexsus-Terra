...
public class RoofBuilder {
    ...
    public static List<BlockPlacement> pitched(List<int[]> interior, int roofBaseY, Material roofMaterial, int worldMaxY) {
        ...
        for (int[] xz : interior) {
            minX = Math.min((int)minX, (int)xz[0]);
            maxX = Math.max((int)maxX, (int)xz[0]);
            minZ = Math.min((int)minZ, (int)xz[1]);
            maxZ = Math.max((int)maxZ, (int)xz[1]);
        }
        int width = maxX - minX + 1;
        int depth = maxZ - minZ + 1;
        boolean ridgeAlongZ = width <= depth;
        double centre = ridgeAlongZ ? (double)(minX + maxX) / 2.0 : (double)(minZ + maxZ) / 2.0;
        double halfSpan = Math.max((double)1.0, (double)((double)(ridgeAlongZ ? width : depth) / 2.0));
        int rise = Math.min((int)9, (int)((int)Math.ceil((double)halfSpan)));
        for (int[] xz : interior) {
            double along = ridgeAlongZ ? (double)xz[0] : (double)xz[1];
            double t = 1.0 - Math.min((double)1.0, (double)(Math.abs((double)(along - centre)) / halfSpan));
            int h = (int)Math.round((double)(t * (double)rise));
            for (int d = 0; d <= 1; ++d) {
                int y = roofBaseY + h - d;
