...
public class BuildingStyle {
    ...
    private static Material windowMaterialFor(OsmFeature feature) {
        String subtype = feature.subtype();
        if (subtype == null) return Material.GLASS;
        switch (subtype) {
            case "commercial":
            case "retail":
            case "office":
    ...
    private static Material roofMaterialFor(OsmFeature feature) {
        ...
        switch (subtype) {
            ...
            case "dormitory":
            case "terrace": {
                Material[] variants = {Material.BROWN_TERRACOTTA, Material.GRAY_CONCRETE, Material.BLACK_CONCRETE, Material.WEATHERED_COPPER};
                return variants[(hashFeature(feature) & 0x7fffffff) % variants.length];
            }
            case "commercial":
            case "retail":
            case "supermarket":
            case "office": {
                Material[] variants = {Material.LIGHT_GRAY_CONCRETE, Material.WHITE_CONCRETE, Material.GRAY_CONCRETE, Material.EXPOSED_COPPER};
                return variants[(hashFeature(feature) & 0x7fffffff) % variants.length];
            }
            case "hotel": {
                Material[] variants = {Material.POLISHED_DEEPSLATE, Material.OXIDIZED_COPPER, Material.WEATHERED_COPPER, Material.CUT_COPPER};
                return variants[(hashFeature(feature) & 0x7fffffff) % variants.length];
            }
            case "industrial":
            case "warehouse":
