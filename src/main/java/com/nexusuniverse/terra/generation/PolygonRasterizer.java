...
public class PolygonRasterizer {
    ...
    public static List<int[]> outline(List<double[]> closedPolygon) {
        ...
        while (i + 1 < closedPolygon.size()) {
            double[] a = (double[])closedPolygon.get(i);
            double[] b = (double[])closedPolygon.get(i + 1);
            double dx = b[0] - a[0];
            double dz = b[1] - a[1];
            double length = Math.sqrt((double)(dx * dx + dz * dz));
            int steps = Math.max((int)1, (int)((int)Math.ceil((double)length)));
            for (int s = 0; s <= steps; ++s) {
                int z;
                double t = (double)s / (double)steps;
                int x = (int)Math.round((double)(a[0] + dx * t));
                long key = PolygonRasterizer.packXZ(x, z = (int)Math.round((double)(a[1] + dz * t)));
                if (unique.containsKey((Object)key)) continue;
                unique.put((Object)key, (Object)new int[]{x, z, along++});
            }
            ++i;
        }
        return new ArrayList(unique.values());
