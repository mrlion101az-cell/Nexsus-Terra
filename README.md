...
## Usage
1. `mvn package`
2. Copy `target/NexusTerra-0.1.30.jar` into your server's `plugins/` folder (delete the old version first).
...
## v0.1.27 — full-height clearing, expansions survive disconnects

**Clearing now reaches the world build limit by default.** The clear pass only ever cleared from
`groundY+1` up to `groundY + clear-height` (default 24 blocks). `groundY` comes from real-world
elevation data, which sits well below whatever tall Minecraft terrain (mountains, trees) already
existed at that spot -- so anything taller than 24 blocks above the real ground level was left
standing, poking through the generated terrain. `generation.clear-height` in `config.yml` now
defaults to `-1`, which means "clear the entire column above ground up to the world's build
height limit," not a fixed 24-block band. This applies to both `/nexusterra generate` and every
tile of `/nexusterra expand`. Set a positive number instead only if you specifically want a
capped/faster clear in terrain you know is flat -- it'll leave tall existing terrain sticking out
again if you do.

Worth knowing: clearing a full ~380-block column (with the default 1.21 world height) is
meaningfully more work per column than the old 24-block band, especially over the many thousands
of columns in a 150m+ tile. Expect clearing to take noticeably longer per tile than before --
`generation.clear-checks-per-tick` (default 12000) controls how much of that work happens per
server tick if you want to trade off clearing speed against tick impact on other players.

**Expansions now keep running if you disconnect.** Previously `ExpansionManager` held onto the
live `Player` object from the moment `/nexusterra expand` was run, and checked `isOnline()` before
every tile -- the instant that came back false, the whole run stopped and the session was removed,
even mid-tile. Rewritten so a session is tracked and driven purely by the player's UUID: the tile
sequencing (fetching, clearing, placing, advancing to the next tile) never checks online status at
all anymore, so a long run now genuinely continues with nobody connected. Every progress/status
message now looks the player up fresh via their UUID at the moment it's about to send, and just
skips silently if they're offline right then -- so you'll see progress resume naturally the moment
you reconnect, rather than the run having quietly died. `/nexusterra stop` still works the same
way whether you're still connected, reconnected after being away, or someone else entirely (same
permission) -- it's found by UUID either way. If truly nobody ever comes back to stop it,
`expansion.max-radius-metres` is what ends it on its own, same as before.

## v0.1.28 — fix real build failure from v0.1.27

`mvn package` against the real Paper API failed on v0.1.27:

    ExpansionManager.java:[170,36] <anonymous ...ExpansionManager$1> is not abstract and does
    not override abstract method name() in org.bukkit.command.CommandSender

I don't have a real Paper build environment available in my own sandbox to compile against
(no network access to `repo.papermc.io`), so v0.1.27 was only verified against a hand-written
stub of the Bukkit/Adventure API I'd built for that purpose -- and that stub was incomplete: real
`CommandSender` extends Adventure's `Audience` and Bukkit's `Permissible`, both of which declare
more abstract methods (`name()` among them) than my stub knew about. The anonymous `CommandSender`
I wrote in `ExpansionManager` (added in v0.1.27 so a running tile could still notify the player
after a disconnect/reconnect) only implemented the two `sendMessage` overloads, which was enough
to satisfy my stub but not the real interface.

Fixed by removing that anonymous class entirely rather than trying to chase every abstract method
across `Audience`/`Permissible` by hand (that surface differs across server versions and is
exactly the kind of thing worth not hand-maintaining). `ExpansionManager` now looks the player up
fresh right when each tile is about to start and passes either the real online `Player` or, if
they're offline at that exact moment, `Bukkit`'s console sender -- both are complete, real
`CommandSender` implementations supplied by the server itself, so there's nothing left for this
plugin to implement. The only behavioural difference from the v0.1.27 design: if the player
disconnects mid-tile (after it started, before it finished), they miss that one tile's two status
messages ("site cleared" / "tile complete") instead of picking them up on reconnect -- the run
itself, and every other status message (progress, start, stop, run complete), is unaffected and
still resolves the player fresh via UUID at send time as described above.

Thank you for actually running `mvn package` and sending back the real error -- that's exactly the
kind of gap a stub-only verification can't catch, and it's fixed now.

## v0.1.29 — clearing still capped after v0.1.27? check your live config.yml

If clearing still isn't reaching the top of tall terrain after updating past v0.1.27, this is
almost certainly why: **Bukkit/Paper never rewrites a config.yml that already exists on disk.**
`saveDefaultConfig()` only creates the file the very first time NexusTerra is installed -- once
`plugins/NexusTerra/config.yml` exists on your server, updating the jar to a newer version does
NOT update that file's contents, even if the new version's bundled default changed. v0.1.27
changed `generation.clear-height`'s default from `24` to `-1` (full-height clearing), but that
new default only applies to a *fresh* install. If NexusTerra was already running on your server
before v0.1.27, your live `config.yml` still has the old `clear-height: 24` line sitting in it
from whenever it was first generated, and every version since (including v0.1.28) has been
reading that stale `24` off disk regardless of what the jar's own bundled default says.

**Fix: open your server's `plugins/NexusTerra/config.yml` directly and change**
...
No rebuild or restart needed -- `/nexusterra generate` and `/nexusterra expand` both re-read
`config.yml` from disk on every single run, so the very next command you run after saving that
edit will use the new value.

v0.1.29 adds a startup log line specifically so this doesn't stay invisible again: on enable,
NexusTerra now logs the `clear-height` value actually in effect, and warns loudly (with the exact
fix) if it isn't `-1`. Check your console after a restart -- if this was the cause, it'll say so
directly instead of you having to guess.

## v0.1.30 — server-wide broadcasts, and a real disconnect bug fixed in plain /nexusterra generate

**Everything now goes to every online player, not just whoever ran the command.** Start, per-tile
progress, tile/run completion, failures, and `/nexusterra stop` all broadcast to the whole server
now -- both for `/nexusterra expand` and plain `/nexusterra generate`. This also doubles as an easy
way to verify a long expansion is still alive after the person who started it disconnects: anyone
else online just watches chat.

**On `/nexusterra expand` specifically:** the tile sequencing itself (`ExpansionManager.advance` /
`onTileComplete` and every Bukkit task in between) has never checked whether anyone is online since
v0.1.28 -- Bukkit's scheduler runs on the server's own tick loop for as long as the plugin is
enabled, completely independent of player connections, so there was nothing in the control flow
gating on a player being present. If a run looked stalled after someone disconnected, the two real
explanations are almost always (a) it's still inside a per-tile OSM/elevation fetch or the
`expansion.pacing-seconds` delay between tiles -- both can take a while with no visible output --
or (b) the running jar predates v0.1.28. To make this verifiable rather than something you have to
take on faith, every tile transition (fetch started, data ready, tile placed, advancing) now logs a
line to the **server console** -- watch that during a disconnect test; console output doesn't
depend on chat or any player being online, so it's the definitive way to confirm a run is still
progressing.

**Found and fixed a real, separate disconnect bug in plain `/nexusterra generate`:** unlike
`expand`, the single-shot command was checking `player.isOnline()` after the data fetch completed
and, if the player had disconnected during that fetch, **abandoning the entire generation** without
ever placing a single block -- silently, with just a console warning. This never showed up in any
`expand` testing since `expand` doesn't have (and never had) that check. Removed it: a plain
`/nexusterra generate` now always finishes building regardless of whether the person who ran it is
still connected, same as `expand` already did.

Under the hood, `BlockPlacementTask` no longer takes a `CommandSender` to notify -- it takes a
`Consumer<Component>`, so callers can broadcast, message one player, or anything else without this
class needing to know or care. That's also what let the "player-or-console-sender" workaround from
v0.1.28 be removed entirely -- there's no longer a CommandSender to obtain in the first place.
